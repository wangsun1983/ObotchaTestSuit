...
void testHashMapLongLong() {
    while(1) {
      ...
      JsonWriter writer = JsonWriter::New("./tmp/hashmap_Long_Long_test1.json");
      JsonValue value = JsonValue::New();
      value->importFrom(maps);
      writer->write(value);

      JsonReader reader = JsonReader::New()->loadFile(File::New("./tmp/hashmap_Long_Long_test1.json"));
    ...
    while(1) {
      ...
      JsonWriter writer = JsonWriter::New("./tmp/hashmap_Long_Long_test2.json");
      JsonValue value = JsonValue::New();
      value->importFrom(data);
      writer->write(value);

      JsonReader reader = JsonReader::New()->loadFile(File::New("./tmp/hashmap_Long_Long_test2.json"));
      JsonValue value2 = reader->get();
