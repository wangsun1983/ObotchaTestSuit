...
void testArrayListDouble() {
    while(1) {
      ...
      JsonWriter writer = JsonWriter::New("./tmp/list_Double_test1.json");
      JsonValue value = JsonValue::New();
      value->importFrom(list);
      writer->write(value);

      JsonReader reader = JsonReader::New()->loadFile(File::New("./tmp/list_Double_test1.json"));
    ...
    while(1) {
      ...
      JsonWriter writer = JsonWriter::New("./tmp/list_Double_test2.json");
      JsonValue value = JsonValue::New();
      value->importFrom(list);
      writer->write(value);

      JsonReader reader = JsonReader::New()->loadFile(File::New("./tmp/list_Double_test2.json"));
      JsonValue value2 = reader->get();
