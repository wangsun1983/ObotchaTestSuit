...
void testMapList() {
    while(1) {
      ...
      JsonWriter writer = JsonWriter::New("./tmp/userdata_list_map_test2.json");
      JsonValue value = JsonValue::New();
      value->importFrom(data);
      writer->write(value);

      JsonReader reader = JsonReader::New()->loadFile(File::New("./tmp/userdata_list_map_test2.json"));
      ...
      if(ll == nullptr || ll->size() != 3) {
        TEST_FAIL("[JsonReflect Map And List UserData Test case1]  ");
        break;
      }

      if(ll->get(0)->data1 != 1 || ll->get(0)->data2 != 2) {
        TEST_FAIL("[JsonReflect Map And List UserData Test case2]  ");
        break;
      }

      if(ll->get(1)->data1 != 3 || ll->get(1)->data2 != 4) {
        TEST_FAIL("[JsonReflect Map And List UserData Test case3]  ");
        break;
      }

      if(ll->get(2)->data1 != 5 || ll->get(2)->data2 != 6) {
        TEST_FAIL("[JsonReflect Map And List UserData Test case4]  ");
        break;
      }
      ...
      if(mm->size() != 3) {
        TEST_FAIL("[JsonReflect Map And List UserData Test case5]  ");
        break;
      }

      if(mm->get("a") == nullptr ||mm->get("a")->data1 != 1 ||mm->get("a")->data2 != 2) {
        TEST_FAIL("[JsonReflect Map And List UserData Test case6]  ");
        break;
      }

      if(mm->get("b") == nullptr ||mm->get("b")->data1 != 3 ||mm->get("b")->data2 != 4) {
        TEST_FAIL("[JsonReflect Map And List UserData Test case7]  ");
        break;
      }

      if(mm->get("c") == nullptr ||mm->get("c")->data1 != 5 ||mm->get("c")->data2 != 6) {
        TEST_FAIL("[JsonReflect Map And List UserData Test case8]  ");
        break;
      }
