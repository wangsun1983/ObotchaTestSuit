#include <stdio.h>
#include <unistd.h>

#include "Thread.hpp"
#include "HashMap.hpp"
#include "Boolean.hpp"
#include "TestLog.hpp"

using namespace obotcha;

DECLARE_CLASS(TT6) {
public:
  bool i;
};

void testHashMapBoolean() {

    //void put(T t,U u)/get(T t)
    while(1) {
        HashMap<Boolean,TT6> map = HashMap<Boolean,TT6>::New();
        Boolean tag = Boolean::New(true);
        TT6 tt = TT6::New();
        tt->i = true;
        map->put(tag,tt);
        if(map->size() != 1) {
            TEST_FAIL("[HashMap Boolean Test {put(T t,U u)/get(T t)} case1]");
            break;
        }

        TT6 tt2 = map->get(tag);
        if(tt2 == nullptr || tt2->i != true) {
          TEST_FAIL("[HashMap Boolean Test {put(T t,U u)/get(T t)} case2]");
          break;
        }

        map->put(Boolean::New(false),nullptr);
        if(map->size() != 2) {
          TEST_FAIL("[HashMap Boolean Test {put(T t,U u)/get(T t)} case3]");
          break;
        }

        TT6 TT6 = map->get(nullptr);
        if(TT6 != nullptr) {
          TEST_FAIL("[HashMap Boolean Test {put(T t,U u)/get(T t)} case4]");
          break;
        }

        TEST_OK("[HashMap Boolean Test {put(T t,U u)/get(T t)} case5]");
        break;
    }

    //void remove(T t)
    while(1) {
      HashMap<Boolean,TT6> map = HashMap<Boolean,TT6>::New();
      Boolean tag = Boolean::New(true);
      TT6 tt = TT6::New();
      tt->i = 100;
      map->put(tag,tt);

      int size = map->size();
      map->remove(Boolean::New(true));
      if(size != 1 && map->size() != 0) {
        TEST_FAIL("[HashMap Boolean Test {remove(T t)} case1]");
        break;
      }

      map->put(tag,tt);
      size = map->size();
      map->remove(nullptr);
      if(map->size() != 1) {
        TEST_FAIL("[HashMap Boolean Test {remove(T t)} case2]");
        break;
      }

      map->remove(Boolean::New(true));
      if(map->size() != 0) {
        TEST_FAIL("[HashMap Boolean Test {remove(T t)} case3]");
        break;
      }

      TEST_OK("[HashMap Boolean Test {remove(T t)} case4]");
      break;
    }

    //bool isEmpty()
    while(1) {
      HashMap<Boolean,TT6> map = HashMap<Boolean,TT6>::New();
      Boolean tag = Boolean::New(true);
      TT6 tt = TT6::New();
      tt->i = 100;
      map->put(tag,tt);

      if(map->isEmpty()) {
        TEST_FAIL("[HashMap Boolean Test {isEmpty()} case1]");
        break;
      }

      map->remove(Boolean::New(true));
      if(!map->isEmpty()) {
        TEST_FAIL("[HashMap Boolean Test {isEmpty()} case2]");
        break;
      }

      TEST_OK("[HashMap Boolean Test {isEmpty()} case3]");
      break;
    }

    //void clear()
    while(1) {
      HashMap<Boolean,TT6> map = HashMap<Boolean,TT6>::New();
      Boolean tag = Boolean::New(true);
      TT6 tt = TT6::New();
      tt->i = true;
      map->put(tag,tt);
      map->put(Boolean::New(false),tt);

      int size = map->size();
      map->clear();
      if(size != 2 || map->size() != 0) {
        TEST_FAIL("[HashMap Boolean Test {clear()} case1]");
        break;
      }

      HashMap<Boolean,TT6> map2 = HashMap<Boolean,TT6>::New();
      map2->clear();
      if(map2->size() != 0) {
        TEST_FAIL("[HashMap Boolean Test {clear()} case2]");
        break;
      }

      TEST_OK("[HashMap Boolean Test {clear()} case3]");
      break;
    }

    //int size()
    while(1) {
      HashMap<Boolean,TT6> map = HashMap<Boolean,TT6>::New();
      Boolean tag = Boolean::New(true);
      TT6 tt = TT6::New();
      tt->i = true;
      map->put(tag,tt);
      map->put(Boolean::New(false),tt);
      if(map->size() != 2 ) {
        TEST_FAIL("[HashMap Boolean Test {size()} case1]");
        break;
      }

      TEST_OK("[HashMap Boolean Test {size()} case1]");
      break;
    }

    //ArrayList<V> keySet()
    while(1) {
      HashMap<Boolean,TT6> map = HashMap<Boolean,TT6>::New();
      TT6 tt1 = TT6::New();
      tt1->i = true;
      map->put(Boolean::New(true),tt1);

      TT6 tt2 = TT6::New();
      tt2->i = false;
      map->put(Boolean::New(false),tt2);

      ArrayList<Boolean> keys = map->keySet();
      int size = keys->size();
      if(size != 2) {
        TEST_FAIL("[HashMap Boolean Test {keySet()} case1]");
        break;
      }

      for(int index = 0;index < 2;index++) {
        Boolean key1 = keys->get(index);
        if(map->get(key1) == nullptr) {
          //TEST_FAIL("key1->int is %d,index is %d ",key1->toValue(),index);
          TEST_FAIL("[HashMap Boolean Test {keySet()} case2]");
          break;
        }
      }

      HashMap<Boolean,TT6> map2 = HashMap<Boolean,TT6>::New();
      ArrayList<Boolean>keys2 = map2->keySet();
      if(keys2->size() != 0) {
          TEST_FAIL("[HashMap Boolean Test {keySet()} case3]");
          break;
      }

      TEST_OK("[HashMap Boolean Test {keySet()} case4]");
      break;
    }

    //ArrayList<V> entrySet()
    while(1) {
      HashMap<Boolean,TT6> map = HashMap<Boolean,TT6>::New();
      TT6 tt1 = TT6::New();
      tt1->i = true;
      map->put(Boolean::New(true),tt1);

      TT6 tt2 = TT6::New();
      tt2->i = false;
      map->put(Boolean::New(false),tt2);

      ArrayList<TT6> keys = map->entrySet();
      int size = keys->size();
      for(int index = 0;index < size;index++) {
        TT6 key1 = keys->get(index);
        if(map->get(Boolean::New(key1->i)) == nullptr) {
          TEST_FAIL("[HashMap Boolean Test {entrySet()} case1]");
          break;
        }
      }

      HashMap<Boolean,TT6> map2 = HashMap<Boolean,TT6>::New();
      ArrayList<TT6>values = map2->entrySet();
      if(values->size() != 0) {
          TEST_FAIL("[HashMap Boolean Test {entrySet()} case2]");
          break;
      }

      TEST_OK("[HashMap Boolean Test {entrySet()} case3]");
      break;
    }


    //_MapIterator(getKey/getValue)
    while(1) {
      HashMap<Boolean,TT6> map = HashMap<Boolean,TT6>::New();
      TT6 tt1 = TT6::New();
      tt1->i = true;
      map->put(Boolean::New(true),tt1);

      TT6 tt2 = TT6::New();
      tt2->i = false;
      map->put(Boolean::New(false),tt2);

      int index = 0;
      MapIterator<Boolean,TT6> iterator = map->getIterator();
      while(iterator->hasValue()) {
          Boolean key = iterator->getKey();
          if(map->get(key) == nullptr) {
            TEST_FAIL("[MapIterator Boolean Test {getKey/getValue()} case1]");
            break;
          }

          TT6 t1 = iterator->getValue();
          if(t1->i != key->toValue()) {
            TEST_FAIL("[MapIterator Boolean Test {getKey/getValue()} case2]");
            break;
          }
          iterator->next();
          index++;
      }

      TEST_OK("[MapIterator Boolean Test {getKey/getValue()} case3]");
      break;

    }


}
