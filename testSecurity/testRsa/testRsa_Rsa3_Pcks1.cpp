...
void testRsa3Pcks1() {
  //create test data
  File data = File::New("./tmp/Rsa3_pcks1_data");
  if(!data->exists()) {
    data->createNewFile();
  }
  ...
  while(1) {
    ...
    secretKey->generate(String::New("./tmp/test_Rsa3_pcks1_encrypt_deckey"),
                        String::New("./tmp/test_Rsa3_pcks1_encrypt_enckey"),
                        key);
    ...
    while(1) {
      ...
      String result = md5sum->encodeFile(File::New("./tmp/rsa3_pcks1_encrypt_outdata_dec"));
